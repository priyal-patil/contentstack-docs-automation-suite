#!/usr/bin/env bash
# =============================================================================
# CMS Users and Roles — 7 flows in CRUD sequence, 1 worker, 5 min cap
#
# Execution order (CRUD):
#   1.  create-a-role
#   2.  add-a-new-user
#   3.  examples-to-create-custom-roles-scenario-1
...
#
# Usage:
#   ./scripts/run-cms-users-and-roles.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-users-and-roles.sh   (headed)
#   REPORT_DIR=reports/my-uar ./scripts/run-cms-users-and-roles.sh
#   PW_RETRIES=0 ./scripts/run-cms-users-and-roles.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-users-and-roles-$TS}"
LOG="$REPORT_DIR/users-and-roles-run.log"
...
USERS_AND_ROLES_FLOWS=(
  "create-a-role"
  "add-a-new-user"
  "examples-to-create-custom-roles-scenario-1"
...
TOTAL_FLOWS=${#USERS_AND_ROLES_FLOWS[@]}
PASS_COUNT=0
FAIL_COUNT=0
TOTAL_EXIT=0
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Users and Roles — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${USERS_AND_ROLES_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Users and Roles — COMPLETE" | tee -a "$LOG"
