...
#   SKIP_SLACK=1 ./scripts/run-cms-headless-report.sh   # skip Slack post (summary files still generated by urlRunSummary)
# Background:
...
# Post flow counts + sample failures to Slack (if SLACK_BOT_TOKEN or slack.authToken is set). SKIP_SLACK=1 to disable.
npx ts-node "$ROOT/scripts/urlRunSummaryAndSlack.ts" --reportDir "$REPORT_DIR" 2>&1 | tee -a "$REPORT_DIR/cms-playwright.log" || true
