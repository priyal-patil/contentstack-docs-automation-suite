#!/usr/bin/env bash
# =============================================================================
# CMS Tokens — 6 flows in CRUD sequence, 1 worker, 5 min cap
#
# Execution order (CRUD):
#   1.  create-a-delivery-token
#   2.  generate-a-management-token
#   3.  edit-a-delivery-token
#   4.  edit-a-management-token
#   5.  delete-a-delivery-token
#   6.  delete-a-management-token
#
...
#
# Usage:
#   ./scripts/run-cms-tokens.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-tokens.sh   (headed)
#   REPORT_DIR=reports/my-tokens ./scripts/run-cms-tokens.sh
#   PW_RETRIES=0 ./scripts/run-cms-tokens.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-tokens-$TS}"
LOG="$REPORT_DIR/tokens-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Tokens — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${TOKENS_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Tokens — COMPLETE" | tee -a "$LOG"
