#!/usr/bin/env bash
# =============================================================================
# CMS Webhook — 7 flows in CRUD sequence, 1 worker, 5 min cap
#
# Execution order (CRUD):
#   1.  create-a-webhook
#   2.  edit-a-webhook
#   3.  enable-or-disable-a-webhook
#   4.  view-webhook-logs
#   5.  export-a-webhook
#   6.  import-a-webhook
#   7.  delete-a-webhook
...
#
# Usage:
#   ./scripts/run-cms-webhook.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-webhook.sh   (headed)
#   REPORT_DIR=reports/my-webhook ./scripts/run-cms-webhook.sh
#   PW_RETRIES=0 ./scripts/run-cms-webhook.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-webhook-$TS}"
LOG="$REPORT_DIR/webhook-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Webhook — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${WEBHOOK_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
  ...
  [[ "${FLOW_ID}" == "create-a-webhook" ]] && FLOW_TIMEOUT_MINUTES=10
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Webhook — COMPLETE" | tee -a "$LOG"
