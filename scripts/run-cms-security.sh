#!/usr/bin/env bash
# =============================================================================
# CMS Security — 6 flows in CRUD sequence, 1 worker, 5 min cap
#
...
#
# Usage:
#   ./scripts/run-cms-security.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-security.sh   (headed)
#   REPORT_DIR=reports/my-security ./scripts/run-cms-security.sh
#   PW_RETRIES=0 ./scripts/run-cms-security.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-security-$TS}"
LOG="$REPORT_DIR/security-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Security — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${SECURITY_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Security — COMPLETE" | tee -a "$LOG"
