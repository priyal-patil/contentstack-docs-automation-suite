#!/usr/bin/env bash
# =============================================================================
# CMS Live Preview — 7 flows in CRUD sequence, 1 worker, 5 min cap
#
# Execution order (CRUD):
#   Create / Setup:
#   1.  set-up-live-preview-for-your-stack
#   Read:
#   2.  open-live-preview-in-a-new-tab
#   3.  preview-content-on-different-environments
#   4.  preview-content-across-multiple-channels
#   5.  preview-content-across-varied-window-sizes
#   Update:
#   6.  custom-preview-urls
#   7.  track-and-edit-content-in-real-time
#
...
#
# Usage:
#   ./scripts/run-cms-live-preview.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-live-preview.sh   (headed)
#   REPORT_DIR=reports/my-live-preview ./scripts/run-cms-live-preview.sh
#   PW_RETRIES=0 ./scripts/run-cms-live-preview.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-live-preview-$TS}"
LOG="$REPORT_DIR/live-preview-run.log"
...
TOTAL_FLOWS=${#LIVE_PREVIEW_FLOWS[@]}
PASS_COUNT=0
FAIL_COUNT=0
TOTAL_EXIT=0
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Live Preview — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${LIVE_PREVIEW_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Live Preview — COMPLETE" | tee -a "$LOG"
