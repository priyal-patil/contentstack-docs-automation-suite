#!/usr/bin/env bash
# =============================================================================
# CMS Search — 13 flows in CRUD sequence, 1 worker, 5 min cap
#
...
#   4.  advanced-search
#   5.  use-filters
#   6.  get-localized-entries
#   7.  about-localization-operator
#   Create:
#   8.  save-your-views
#   9.  shared-views
#   Update:
#   10. use-saved-views
#   11. publish-entries-and-assets-in-bulk
#   12. unpublish-entries-and-assets-in-bulk
#   Delete:
#   13. delete-entries-and-assets-in-bulk
#
...
#
# Usage:
#   ./scripts/run-cms-search.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-search.sh   (headed)
#   REPORT_DIR=reports/my-search ./scripts/run-cms-search.sh
#   PW_RETRIES=0 ./scripts/run-cms-search.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-search-$TS}"
LOG="$REPORT_DIR/search-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Search — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${SEARCH_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Search — COMPLETE" | tee -a "$LOG"
