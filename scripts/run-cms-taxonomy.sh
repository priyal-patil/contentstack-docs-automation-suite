#!/usr/bin/env bash
# =============================================================================
# CMS Taxonomy — 12 flows in CRUD sequence, 1 worker, 5 min cap
#
...
#   3.  edit-a-taxonomy
#   4.  edit-a-term
#   5.  move-or-reorder-a-term
#   6.  add-taxonomy-to-a-content-type
#   7.  export-a-taxonomy
#   8.  import-a-taxonomy
#   9.  taxonomy-for-a-basic-blog-website
#  10.  setting-up-taxonomy-based-permissions-for-regional-content-management
#   Delete:
#  11.  delete-a-term
#  12.  delete-a-taxonomy
#
...
#
# Usage:
#   ./scripts/run-cms-taxonomy.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-taxonomy.sh   (headed)
#   REPORT_DIR=reports/my-taxonomy ./scripts/run-cms-taxonomy.sh
#   PW_RETRIES=0 ./scripts/run-cms-taxonomy.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-taxonomy-$TS}"
LOG="$REPORT_DIR/taxonomy-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Taxonomy — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${TAXONOMY_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Taxonomy — COMPLETE" | tee -a "$LOG"
