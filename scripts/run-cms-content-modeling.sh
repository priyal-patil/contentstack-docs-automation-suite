...
#
# Usage:
#   ./scripts/run-cms-content-modeling.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-content-modeling.sh   (headed)
#   REPORT_DIR=reports/my-cm ./scripts/run-cms-content-modeling.sh
#   PW_RETRIES=0 ./scripts/run-cms-content-modeling.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-content-modeling-$TS}"
LOG="$REPORT_DIR/content-modeling-run.log"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Content Modeling — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${CONTENT_MODELING_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Content Modeling — COMPLETE" | tee -a "$LOG"
