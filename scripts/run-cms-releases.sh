#!/usr/bin/env bash
# =============================================================================
# CMS Releases — 11 flows in CRUD sequence, 1 worker, 5 min cap
#
# Execution order (CRUD):
#   1.  create-a-new-release
#   2.  add-entry-asset-to-a-release
#   3.  add-entry-asset-to-a-release-part-2
#   4.  edit-a-release
#   5.  lock-a-release
#   6.  unlock-a-release
#   7.  deploy-a-release
#   8.  clone-a-release
#   9.  update-release-items-to-their-latest-versions
#   10. remove-entry-asset-from-a-release
#   11. delete-a-release
#
...
#
# Usage:
#   ./scripts/run-cms-releases.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-releases.sh   (headed)
#   REPORT_DIR=reports/my-releases ./scripts/run-cms-releases.sh
#   PW_RETRIES=0 ./scripts/run-cms-releases.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-releases-$TS}"
LOG="$REPORT_DIR/releases-run.log"
...
RELEASES_FLOWS=(
  ...
  "add-entry-asset-to-a-release-part-2"
  "edit-a-release"
  "lock-a-release"
  "unlock-a-release"
  "deploy-a-release"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Releases — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${RELEASES_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
  ...
  [[ "${FLOW_ID}" == "create-a-new-release" ]] && FLOW_TIMEOUT_MINUTES=10
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Releases — COMPLETE" | tee -a "$LOG"
