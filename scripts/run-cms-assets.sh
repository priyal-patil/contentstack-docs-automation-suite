#!/usr/bin/env bash
# =============================================================================
# CMS Assets — 17 flows in CRUD sequence, 1 worker, 5 min cap
#
...
#   4.  rename-a-folder
#   5.  move-a-folder
#   6.  move-assets-to-folder-in-bulk
#   7.  publish-an-asset
#   8.  unpublish-an-asset
#   9.  bulk-publish-assets
#  10.  bulk-unpublish-assets
#  11.  generate-a-permanent-asset-url
#  12.  name-asset-versions
#  13.  rename-asset-versions
#  14.  restore-old-asset-version
#   Delete:
#  15.  delete-an-asset
#  16.  delete-a-folder
#  17.  bulk-delete-assets
#
...
#
# Usage:
#   ./scripts/run-cms-assets.sh
#   PLAYWRIGHT_HEADLESS=0 ./scripts/run-cms-assets.sh   (headed)
#   REPORT_DIR=reports/my-assets ./scripts/run-cms-assets.sh
#   PW_RETRIES=0 ./scripts/run-cms-assets.sh            (disable retries)
# =============================================================================
...
TS="$(date +%Y%m%d-%H%M%S)"
export REPORT_DIR="${REPORT_DIR:-$ROOT/reports/cms-assets-$TS}"
LOG="$REPORT_DIR/assets-run.log"
...
ASSETS_FLOWS=(
  ...
  "rename-a-folder"
  "move-a-folder"
  "move-assets-to-folder-in-bulk"
  "publish-an-asset"
  "unpublish-an-asset"
  "bulk-publish-assets"
  "bulk-unpublish-assets"
...
: > "$LOG"
echo "================================================================" | tee -a "$LOG"
echo "  CMS Assets — ${TOTAL_FLOWS} flows | 1 worker | ${PW_FLOW_MAX_MINUTES} min cap" | tee -a "$LOG"
...
IDX=0
for FLOW_ID in "${ASSETS_FLOWS[@]}"; do
  IDX=$((IDX + 1))
  FLOW_START="$(date +%s)"
...
echo "================================================================" | tee -a "$LOG"
echo "  CMS Assets — COMPLETE" | tee -a "$LOG"
